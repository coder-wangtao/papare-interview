...
1.强制缓存优先于协商缓存进行，若强制缓存(Expires 和 Cache-Contro)生效则直接使用缓存，若不生效则进行协商缓存(Last-Modified /lf-Modified-Since 和 Etag /lf-None-Match) 2.协商缓存由服务器决定是否使用缓存，若协商缓存失效，那么代表该请求的缓存失效，重新获取请求结果，再存入浏览器缓存中;若协商缓存生效则返回 304，继续使用缓存
